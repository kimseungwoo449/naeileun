...
const Calendar = () => {
  ...
  const convertKoreanDateStringToDate = (dateString) => {
    ...
    return new Date(year, monthMap[month], parseInt(day.replace(',', '')));
  };
  ...
  const renderCalendar = () => {
    ...
    while (day.getMonth() === currentDate.getMonth()) {
      ...
      const filteredJobs = jobData.filter(job => {
        const jobDate = convertKoreanDateStringToDate(job.applicationStart);
